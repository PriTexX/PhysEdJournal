...
internal sealed class UserInfoClient : IDisposable
{
    ...
    {
        this._client = new GraphQLHttpClient(userInfoServerUrl, new NewtonsoftJsonSerializer());
    }
    ...
    public async Task<List<Student>> GetStudentsAsync(int limit, int offset)
    {
        ...
        var response = await this._client.SendQueryAsync<PagedGraphQLStudent>(request);
