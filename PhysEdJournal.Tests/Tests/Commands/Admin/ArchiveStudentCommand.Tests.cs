...
public sealed class ArchiveStudentCommandTests : DatabaseTestsHelper
{
    ...
    public async Task ArchiveStudentAsync_ArchivesStudent_ShouldWorkProperly(int additionalPoints)
    {
        ...
        var student = EntitiesFactory.CreateStudent(group.GroupName, lastSemester.Name, false, true, additionalPoints);
        ...
        var archivedStudent = new ArchivedStudentEntity
        {
            ...
            TotalPoints = CalculateTotalPoints(student.Visits, group.VisitValue, 
    ...
    public async Task ArchiveStudentAsync_ArchivesStudentForceMode_ShouldWorkProperly(int additionalPoints)
    {
        //Arrange
        await using var context = CreateContext();
        await ClearDatabase(context);
    ...
    [InlineData(0)]
    [InlineData(int.MinValue)]
    public async Task ArchiveStudentAsync_NotEnoughPointsException_ShouldThrowException(int additionalPoints)
    {
        //Arrange
        await using var context = CreateContext();
        await ClearDatabase(context);
    ...
    public async Task ArchiveStudentAsync_CannotMigrateToNewSemesterException_ShouldThrowException()
    {
        ...
        var currentSemester = EntitiesFactory.CreateSemester("2021-2023/autumn", true);
        var group = EntitiesFactory.CreateGroup("211-729", 2, teacher.TeacherGuid);
        var student = EntitiesFactory.CreateStudent(group.GroupName, currentSemester.Name, false, true, 50);
