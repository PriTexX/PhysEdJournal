...
public sealed class DeletePointsCommandTests : DatabaseTestsHelper
{
    ...
    [InlineData(10)]
    [InlineData(100)]
    public async Task DeletePointsAsync_DeletePointsFromStudent_ShouldWorkProperly(int additionalPoints)
    {
        // Arrange
        ...
        var student = EntitiesFactory.CreateStudent(group.GroupName, semester.Name, false, true, additionalPoints);
        ...
        var historyObj = context.PointsStudentsHistory.FirstOrDefault(h => h.Points == historyEntity.Points);
        ...
        var historyEntityFromDb = studentFromDb.PointsStudentHistory.FirstOrDefault(h => h.Points == historyEntity.Points);
    ...
    public async Task DeletePointsAsync_DeletePointsFromStudentForce_ShouldWorkProperly()
    {
        ...
        var semester = EntitiesFactory.CreateSemester("2022-2023/spring", true);
        var group = EntitiesFactory.CreateGroup("211-729");
        var student = EntitiesFactory.CreateStudent(group.GroupName, semester.Name, false, true, 10);
        var teacher = EntitiesFactory.CreateTeacher(permissions: TeacherPermissions.SuperUser);
        var date = DateOnly.FromDateTime(DateTime.Now).AddDays(-8);
        var historyEntity = EntitiesFactory.CreatePointsStudentHistoryEntity(student.StudentGuid, WorkType.Activist, teacher.TeacherGuid, DateOnlyGenerator.GetWorkingDate(date), 10, semester.Name);
    ...
    [Fact]
    public async Task DeletePointsAsync_StudentNotFoundException_ShouldThrow()
    {
        // Arrange
        await using var context = CreateContext();
        await ClearDatabase(context);
