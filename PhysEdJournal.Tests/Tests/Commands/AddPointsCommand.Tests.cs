...
public sealed class AddPointsCommandTests : DatabaseTestsHelper
{
    ...
    private async Task AddPointsAsync_AddsPointsToStudent_ShouldWorkProperly(WorkType workType)
    {
        ...
        var historyEntity = EntitiesFactory.CreatePointsStudentHistoryEntity(student.StudentGuid, workType, teacher.TeacherGuid, DateOnly.FromDateTime(DateTime.Now), 10);
    ...
    public async Task AddPointsAsync_StudentNotFoundException_ShouldThrowException()
    {
        ...
        var group = EntitiesFactory.CreateGroup("211-729");
        var teacher = EntitiesFactory.CreateTeacher(permissions: TeacherPermissions.SuperUser);
        var historyEntity = EntitiesFactory.CreatePointsStudentHistoryEntity("wrongGuid", WorkType.Activist, teacher.TeacherGuid, DateOnly.FromDateTime(DateTime.Now), 10);
        var payload = new AddPointsCommandPayload
        {
            StudentGuid = historyEntity.StudentGuid,
            Date = historyEntity.Date,
