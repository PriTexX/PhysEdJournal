...
public sealed class AddPointsCommandTests : DatabaseTestsHelper
{
    ...
    public async Task AddPointsAsync_AddsPointsToStudent_ShouldWorkProperly(WorkType workType)
    {
        ...
        var historyEntity = EntitiesFactory.CreatePointsStudentHistoryEntity(student.StudentGuid, workType, teacher.TeacherGuid, DateOnlyGenerator.GetWorkingDate(), 10);
    ...
    [InlineData(WorkType.InternalTeam)]
    [InlineData(WorkType.Competition)]
    public async Task AddPointsAsync_ActionFromFutureException_ShouldThrowException(WorkType workType)
    {
        // Arrange
        await using var context = CreateContext();
        await ClearDatabase(context);
        ...
        var student = EntitiesFactory.CreateStudent(group.GroupName, semester.Name, false, true);
        var teacher = EntitiesFactory.CreateTeacher(permissions: TeacherPermissions.SuperUser);
        var historyEntity = EntitiesFactory.CreatePointsStudentHistoryEntity(student.StudentGuid, workType, teacher.TeacherGuid, DateOnly.MaxValue, 10);
        var payload = new AddPointsCommandPayload
        {
            StudentGuid = historyEntity.StudentGuid,
            Date = historyEntity.Date,
    ...
    public async Task AddPointsAsync_StudentNotFoundException_ShouldThrowException()
    {
        ...
        var group = EntitiesFactory.CreateGroup("211-729");
        var teacher = EntitiesFactory.CreateTeacher(permissions: TeacherPermissions.SuperUser);
        var historyEntity = EntitiesFactory.CreatePointsStudentHistoryEntity("wrongGuid", WorkType.Activist, teacher.TeacherGuid, DateOnlyGenerator.GetWorkingDate(), 10);
        var payload = new AddPointsCommandPayload
        {
            StudentGuid = historyEntity.StudentGuid,
            Date = historyEntity.Date,
    ...
    public async Task AddPointsAsync_PointsExpiredException_ShouldThrowException()
    {
        ...
        var historyEntity = EntitiesFactory.CreatePointsStudentHistoryEntity(student.StudentGuid, WorkType.OnlineWork, teacher.TeacherGuid, DateOnlyGenerator.GetWorkingDate(oldDate), 10);
        var payload = new AddPointsCommandPayload
        {
            StudentGuid = historyEntity.StudentGuid,
            Date = historyEntity.Date,
    ...
    public async Task AddPointsAsync_NonWorkingDayException_ShouldThrowException()
    {
        ...
        var student = EntitiesFactory.CreateStudent(group.GroupName, semester.Name, false, true);
        var teacher = EntitiesFactory.CreateTeacher(permissions: TeacherPermissions.SuperUser);
        var historyEntity = EntitiesFactory.CreatePointsStudentHistoryEntity(student.StudentGuid, WorkType.Activist, teacher.TeacherGuid, DateOnlyGenerator.GetNonWorkingDate(), 10);
        var payload = new AddPointsCommandPayload
        {
            StudentGuid = historyEntity.StudentGuid,
            Date = historyEntity.Date,
